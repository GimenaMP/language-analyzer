...
public class SQLSyntacticAnalyzer implements ISyntacticAnalyzer {
    ...
    public List<AnalysisError> analyze(List<Token> tokens, LanguageType language) {
        ...
        // Solo analizar si es SQL/PLSQL
        if (language != LanguageType.PLSQL) {
            return errors;
        }

        try {
            // Analizar cada sentencia SQL
            ...
            errors.add(new AnalysisError(
                    "Error durante análisis sintáctico: " + e.getMessage(),
    ...
    private void analizarDrop() {
        ...
        if (!esperarTipo("IDENTIFICADOR")) {
            agregarError("Se esperaba identificador después de DROP", obtenerTokenActual());
            return;
        }

        // Verificar si es DROP TABLE
        if (consumirSiExiste("TABLE")) {
            if (!esperarTipo("IDENTIFICADOR")) {
                agregarError("Se esperaba nombre de tabla después de DROP TABLE", obtenerTokenActual());
            }
        } else {
            agregarError("Se esperaba cláusula TABLE después de DROP", obtenerTokenActual());
        }
    ...
    private void analizarCreate() {
        esperarToken("CREATE");
        if (!esperarTipo("IDENTIFICADOR")) {
            agregarError("Se esperaba identificador después de CREATE", obtenerTokenActual());
            return;
        }

        // Verificar si es CREATE TABLE
        if (consumirSiExiste("TABLE")) {
            if (!esperarTipo("IDENTIFICADOR")) {
                agregarError("Se esperaba nombre de tabla después de CREATE TABLE", obtenerTokenActual());
                return;
            }
            // Analizar definición de tabla
            analizarDefinicionTabla();
        } else {
            agregarError("Se esperaba cláusula TABLE después de CREATE", obtenerTokenActual());
        }

    }
    private void analizarDefinicionTabla() {
        if (!consumirSiExiste("PAREN_IZQ")) {
            agregarError("Se esperaba paréntesis izquierdo después de nombre de tabla", obtenerTokenActual());
            return;
        }
        ...
        while (true) {
            if (!esperarTipo("IDENTIFICADOR")) {
                agregarError("Se esperaba nombre de columna", obtenerTokenActual());
                return;
            }

            // Analizar tipo de dato
            if (!esperarTipo("TIPO_DATO")) {
                agregarError("Se esperaba tipo de dato después de nombre de columna", obtenerTokenActual());
                return;
            }

            // Verificar restricciones opcionales
            while (consumirSiExiste("RESTRICCION")) {
                // Aquí podrías analizar restricciones como PRIMARY KEY, NOT NULL, etc.
            }

            if (!consumirSiExiste("COMA")) break; // Salir si no hay más columnas
        }
    ...
    private void analizarDelete() {
        ...
        if (!encontrarClausula("FROM")) {
            agregarError("Se esperaba cláusula FROM después de DELETE", obtenerTokenActual());
            return;
        }

        // Analizar nombre de tabla
        if (!esperarTipo("IDENTIFICADOR")) {
            agregarError("Se esperaba nombre de tabla después de FROM", obtenerTokenActual());
            return;
        }

        // Analizar cláusula WHERE opcional
        if (consumirSiExiste("WHERE")) {
            analizarCondicion();
        }
    ...
    private void analizarUpdate() {
        esperarToken("UPDATE");
        if (!esperarTipo("IDENTIFICADOR")) {
            agregarError("Se esperaba nombre de tabla después de UPDATE", obtenerTokenActual());
            return;
        }

        // Esperar SET obligatorio
        if (!esperarToken("SET")) {
            agregarError("Se esperaba cláusula SET después de UPDATE", obtenerTokenActual());
            return;
        }

        // Analizar lista de asignaciones
        while (true) {
            if (!esperarTipo("IDENTIFICADOR")) {
                agregarError("Se esperaba nombre de columna en SET", obtenerTokenActual());
                return;
            }

            // Esperar operador de asignación
            if (!esperarToken("=")) {
                agregarError("Se esperaba '=' después de nombre de columna", obtenerTokenActual());
                return;
            }

            // Analizar valor
            if (!analizarExpresionColumna()) {
                agregarError("Se esperaba valor después de '='", obtenerTokenActual());
                return;
            }

            if (!consumirSiExiste("COMA")) break; // Salir si no hay más asignaciones
        }
